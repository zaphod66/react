...
class Step7_CustomSpec extends TestKit(ActorSystem("Step7CustomSpec"))
  ...
  test("case1: Random ops reliable") {
...
/*
  test("case1.1: Random ops secondary unreliable") {
    val arbiter = TestProbe()

    val primary = system.actorOf(
      Replica.props(arbiter.ref, Persistence.props(flaky = false)), "case1.1-primary")
    arbiter.expectMsg(Join)
    arbiter.send(primary, JoinedPrimary)

    val secondary = system.actorOf(
      Replica.props(arbiter.ref, Persistence.props(flaky = true)), "case1.1-secondary")
    arbiter.expectMsg(Join)
    arbiter.send(secondary, JoinedSecondary)
...
*/
/*
  test("case1.2: Random ops primary unreliable") {
    val arbiter = TestProbe()

    val primary = system.actorOf(
      Replica.props(arbiter.ref, Persistence.props(flaky = true)), "case1.2-primary")
    arbiter.expectMsg(Join)
    arbiter.send(primary, JoinedPrimary)

    val secondary = system.actorOf(
      Replica.props(arbiter.ref, Persistence.props(flaky = false)), "case1.2-secondary")
    arbiter.expectMsg(Join)
    arbiter.send(secondary, JoinedSecondary)
    ...
    val client = session(primary)
    for (_ <- 0 until 10) randomQuery(client)
  }
*/
/*
